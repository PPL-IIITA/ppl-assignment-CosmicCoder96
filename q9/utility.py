...
def makeCouples(mode,boy_list,girl_list,k):
	...
	for b in boy_list:
		...
		if(len(suitableGirls)>0):
			b.commit()
			suitableGirls[0].commit()
			happiness =0
			compatibility = 0
			temp = Couple(b,suitableGirls[0],happiness,compatibility)
...
def giveGifts(boy_list,girl_list,k):
	...
	for c in couple_list:
		...
		for i in range(0,len(suitableGifts)):
			if moneyLeft < suitableGifts[i].price:
				break
			suitableGifts[i].used = True
			moneySpent += suitableGifts[i].price
			moneyLeft -= suitableGifts[i].price
			totalValue +=  suitableGifts[i].value
			print 'Gift of value ' + str(suitableGifts[i].value) + ' exchanged'

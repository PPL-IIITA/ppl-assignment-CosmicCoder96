...
def getGF(implement, couple_list, boy_list):
	girlFriendList = []
	if (implement == 1):
		committedBoys = []
		committedGirls = []
		for b in couple_list:
			committedBoys.append(b.boy)
			committedGirls.append(b.girl)
		for b in boy_list:
			if b in committedBoys:
				girlFriendList.append(committedGirls[committedBoys.index(b)])
				print committedBoys.index(b)
		return girlFriendList
	elif(implement == 2):
		committedBoys = []
		committedGirls = []
		for b in couple_list:
			committedBoys.append(b.boy)
			committedGirls.append(b.girl)
		committedBoys.sort(key = lambda x: x.name, reverse = False)
		for b in boy_list:
			if binary(b, committedBoys):
				girlFriendList.append(committedGirls[committedBoys.index(b)])
		return girlFriendList
	else:
		committedBoys = {}
		for b in couple_list:
			committedBoys[b.boy] = b.girl
		for b in boy_list:
			try:
				if committedBoys[b]:
					girlFriendList.append(committedBoys[b])
			except:
				continue
		return girlFriendList
def binary(b, committedBoys):
	mid = len(committedBoys)/2
	low = 0
	high = len(committedBoys) -1
	while low <= high:
		mid = (high + low)/2
		if b.name < committedBoys[mid].name:
			high = mid -1
		elif b.name > committedBoys[mid].name:
			low = mid + 1
		else:
			return True
	return False
